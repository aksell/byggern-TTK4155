...
void EEPROM_write(EEPROM_address address, uint8_t data) {
	ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
		while (EECR & (1<<EEWE)) {};			// Wait until  EEPROM Write Enable becomes zero.
		EEAR = address;
		EEDR = data;
		EECR = (EECR & ~(1 << EEWE)) | (1 << EEMWE);// Write a logical one to the EEMWE bit while writing a zero to EEWE in EECR.
