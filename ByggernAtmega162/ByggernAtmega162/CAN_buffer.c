...
bool CAN_buffer_full() { return buffer_full; }

void CAN_buffer_set_full() { buffer_full = true; }
...
bool CAN_buffer_empty() { return buffer_empty; }
...
can_message CAN_buffer_read() {
  ...
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
    ...
    if (tail < CAN_BUFFER_SIZE - 1) {
      message.address =
          convert_from_8_to_16(buffer_data[tail], buffer_data[tail + 1]);
      CAN_buffer_increment_tail();
      CAN_buffer_increment_tail();
    } else {
