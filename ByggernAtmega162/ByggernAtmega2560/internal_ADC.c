...
void internal_ADC_init() {
  // Set MUXER
  ADMUX = (0b11 << REFS0) | (0b0000) |
          (1 << ADLAR); // Select ADC0 and internal Referance and left adjust
  ADCSRB = (0b0 << MUX5);
  ADCSRA =
      (0b1 << ADEN) | (0b110 << ADPS0); // Endable ADC and set prescalar to 64
...
uint8_t internal_ADC_read_free_running_mode() { return ADCH; }
