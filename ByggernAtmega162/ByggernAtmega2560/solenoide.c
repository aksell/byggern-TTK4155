...
void solenoide_init() {
  ...
  PORTH |=
      (1 << SOLENOID_TRIGGER_PIN); // initialize solenoide to retracted position

  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
    TCCR1A =
        (0b00 << COM1A0) | (0b00 << COM1B0) |
        (0b00
         << WGM10); // Toggle OC1A on Compare Match, OC1B and OC1C disconnected
...
void solenoide_trigger(uint16_t solenoide_trigger_time_ms) {
  ...
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
    OCR1A = F_CPU * solenoide_trigger_time_ms * 4 /
            (SOLENOID_PRE_SCALAR * 1000); // Set time interval
    TCNT1 = 0;
