...
void speaker_init() {
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
    DDRL |= (1 << PL3);
    TCCR5A =
        (0b01 << COM5A0) | (0b00 << COM5B0) |
        (0b11
         << WGM50); // Toggle OC1A on Compare Match, OC1B and OC1C disconnected
...
void speaker_on() {
  TCCR5B = (TCCR5B & ~(0b111 << CS50)) |
           (PWM_PRESCALER_BITMAP << CS50); // Set prescalar to 1
}
