...
void dc_motor_controller_enable() { timer0_enable(); }

void dc_motor_controller_dissable() { timer0_dissable(); }
...
void dc_motor_set_refference_ofset(int16_t pos) {
  ...
  int32_t offset = pos * (dc_motor_max_pos / 256) /
                   2; // scale input fraction of working area
  dc_motor_refference_pos = dc_motor_middle_pos + (int16_t)offset;
}

void dc_motor_set_refference_middle() { dc_motor_set_refference_ofset(0); }
...
void dc_motor_PI_controller_update() {
  ...
  int16_t u = (dc_motor_K_p * error) +
              2 * dc_motor_K_i * dc_motor_accumulated_error / PI_FREQUENZY;
